...
export default function SignTwo(props: TCanvasCardProps) {
  const { editable, primary, text } = props;

  const { houseSvg, arrowSvg, windowSvg, phoneSvg, whiteRecSvg } = useGetSvg({
  ...
  return (
    ...
    >
      <Konva.Layer width={1728} height={1152}>
        ...
        <CustomImage x={800.79} y={167.8} svgString={windowSvg} />
        <CustomImage x={1073.84} y={424.29} svgString={arrowSvg} />

        <Konva.Group x={0} y={0}>
          <CustomImage x={303.91} y={829.9} svgString={whiteRecSvg} />
          <CustomImage x={338.15} y={864.1} svgString={phoneSvg} />
